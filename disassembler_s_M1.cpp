...
void I(unsigned int instWord, unsigned int opcode)
{
	...
	if (opcode == 0x13) {	// I instructions
		...
		if (!printMode &&rd) {
			...
			switch (funct3){
				case 0x0: // lb
					regs[rd] = memory[int(I_imm) + temp];
					break;
				case 0x1: //lh
					regs[rd] = (unsigned char)memory[int(I_imm) + temp];
					regs[rd] |= memory[int(I_imm) + temp+1] << 8;
					break;
				case 0x2: //lw
				//	cout << temp << endl;
				//	cout << regs[rd] << endl;
					regs[rd] = (unsigned char)memory[int(I_imm) + temp];
				//	cout << regs[rd] << endl;
					regs[rd] |= (unsigned char)memory[int(I_imm) + temp+1] << 8;
				//	cout << regs[rd] << endl;
					regs[rd] |= (unsigned char)memory[int(I_imm) + temp+2] << 16;
				//	cout << regs[rd] << endl;
					regs[rd] |= memory[int(I_imm) + temp+3] << 24;
				//	cout << regs[rd] << endl;
					break;
				case 0x4: // lbu
					regs[rd] = (unsigned int)memory[int(I_imm) + temp];
					break;
				default: //lhu
					regs[rd] = (unsigned char)memory[int(I_imm) + temp];
					regs[rd] |= (unsigned char)memory[int(I_imm) + temp+1] << 8;
